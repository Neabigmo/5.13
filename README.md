...
- `experiments/`: scripts for witnesses, certificates, Nature-style figures,
...
Use the existing environment and LaTeX toolchain.  The project-local LaTeX
default is `D:\texlive\2025\bin\windows`.
...
The full submission run used for the current package is
`outputs/runs/ml_submission/finalrun_manifest/`.  Data figures are generated by
`experiments/nature_figures.py` using the local
`external/nature-skills/skills/nature-figure/` conventions, with editable SVG,
